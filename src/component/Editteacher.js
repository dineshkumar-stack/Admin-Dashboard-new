...
function Editteacher() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        await axios.put(
          `https://651d9f1b44e393af2d5a19a0.mockapi.io/api/teacher/${params.id}`,
          values
        );
  ...
  useEffect(() => {
    async function fetchData() {
        try {
            let user = await axios.get(`https://651d9f1b44e393af2d5a19a0.mockapi.io/api/teacher/${params.id}`)
